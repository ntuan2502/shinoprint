...
import mediaQueryListFactory from '../common/media-query-list'; // Chiara
...
export default class {
    ...
    // Chiara theme
    initLinkBindChiara() {
        $('.productView-reviewLink').on('click', (event) => {
            if (mediumMedia && mediumMedia.matches) {
                const $tab = $('.productView-tab--reviews');
                if ($tab.length > 0) {
                    event.preventDefault();
                    if (!$tab.hasClass('is-active')) {
                        $tab.find('a').trigger('click');
                    }
                    scrollToElement($tab.get(0), {
                        offset: -100,
                        duration: 100,
                    });
                }
